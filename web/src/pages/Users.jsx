...
export default function Users() {
    // style bases
    const cardBase =
        "rounded-2xl bg-white shadow-lg p-5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl";
    const inputBase =
        "w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 text-sm font-normal";
            ...
            setUsers(
                users.map((user) =>
                    user.id === editingUserId
                        ? { ...user, ...newUser, lastLogin: "Just now" }
                        : user
                )
            );
            setEditingUserId(null);
    ...
    return (
        <div className="px-3 sm:px-6 pt-6 pb-10 text-slate-900">
            ...
            {/* --------------------- Add/Edit User Form --------------------- */}
            {isAddingUser && (
                <div className={`${cardBase} mb-6 transition-all
                    ${entered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
                    style={{ transitionDelay: "30ms" }}>
            ...
            {/* --------------------- Users Table --------------------- */}
            <div className={`${cardBase} transition-all overflow-x-auto
                ${entered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
